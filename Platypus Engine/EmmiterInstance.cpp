...
void EmitterInstance::DrawParticles()
{
	/*for (unsigned int i = 0; i < activeParticles; ++i)
	{
		...
		App->renderer3D->AddParticle(transform, emitterReference->hMaterial.Get(), particle->color, particle->distanceToCamera);
	}*/
}
...
void EmitterInstance::UpdateModules(float dt)
{
	for (int i = 0; i < emitterReference->modules.size(); ++i)
	{
		emitterReference->modules[i]->Update(/*dt, this*/); //Esto tiene que ir asi sin comentar
	}
}